    ...
    const formattedChats = chats.map((chat) => {
      ...
      if (chatTypeFilter === "individual") {
        ...
        if (otherParticipant) {
          ...
          (chatObj as any).recipientProfilePic = (
            otherParticipant as any
          ).profilePic;
          (chatObj as any).unseenCount = unseenCount;
          (chatObj as any).recipientID = otherParticipant._id;
        }
...
export const getChatById = async (req: Request, res: Response) => {
  try {
    ...
    const messagesToUpdate = chat.messages.filter(
      (msg) => !msg.readBy.includes(new mongoose.Types.ObjectId(userId))
    );
    ...
    chat.participants.forEach(async (participantId) => {
      ...
      if (participantIdStr != userId) {
        ...
        await notifyUser({
          ...
          category: "CHAT",
          priority: "HIGH",
          referenceId: new mongoose.Types.ObjectId((chat as any)._id),
          referenceType: "chats",
          metadata: {
...
export const getChatMessages = async (req: Request, res: Response) => {
  try {
    ...
    const populatedMessages =
      populatedChat?.messages.slice(startIndex, endIndex).reverse() || [];
...
// Create a new individual chat or return existing one
export const createOrGetIndividualChat = async (
  req: Request,
  res: Response
) => {
  try {
...
export const addGroupParticipants = async (req: Request, res: Response) => {
  try {
    ...
    chat.participants.push(
      ...newParticipants.map((id) => new mongoose.Types.ObjectId(id))
    );
    await chat.save();
...
export const removeGroupParticipant = async (req: Request, res: Response) => {
  try {
    ...
    if (
      !chat.groupAdmin?.some((adminId) => adminId.toString() === userId) &&
    ...
    if (
      chat.groupAdmin?.some((adminId) => adminId.toString() === participantId)
    ) {
